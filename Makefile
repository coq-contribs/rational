...
OCAMLLIBS?=-I "Rewrite/LeibnizRewrite/MLstuff"\
...
ML4FILES:=Rewrite/LeibnizRewrite/HS/hS_tac.ml4\
  Rewrite/LeibnizRewrite/AC/aC_tac.ml4\
  Rewrite/GenericRewrite/HeadSimpl/tacentry.ml4
...
install-natdynlink:
	cd "." && for i in $(CMXSFILES); do \
	 ...
	 install -m 0755 $$i "$(DSTROOT)"$(COQLIBINSTALL)/Rational/$$i; \
	done
...
uninstall_me.sh: Makefile
	...
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Rational && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Rational" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
